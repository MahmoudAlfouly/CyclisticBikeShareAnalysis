...
# 3. **Specific Analysis Based on User Types**: Analyzing the data based on the two user types to understand how their usage patterns differ.
...
sns.histplot(df['tripduration'], bins=100, kde=True)
...
# There seems to be some outliers that make the visualization less clear. The summary stats confirm this as the 75th percentile of 'tripduration' is about 866 seconds and the max value is much longer than this, I'll filter out the outliers, in order to get a clearer picture of the visualization.
...
# The previous histogram shows the distribution of trip durations. This visualization helps us understand the typical length of bike trips and identify patterns in trip duration. Outliers were filtered to make the visualization clearer.
# 
# ### Insights:
...
# 
# ### Insights:
# Peak times are in accordance with commuting times (around nine a.m and five p.m).

# In[20]:


# Trips distribution by User type
# Plotting
plt.figure(figsize=(10, 6))
...
# This bar chart shows the number of trips taken by each of the two usertypes: customers and subscribers.
# 
# ### Insights:
# Most trips are taken by subscribers with over 300,000 trips. Customers take a minority of trips.
...
# ## Specific Analysis Based on User Types
...
# 
# ### Insights:
# For subscribers, the average trip duration is almost stable in each day of the week. For customers, the day with the lengthiest trips is Thursday.
...
# 
# ### Insight:
# Younger people are more likely to be 'Customer' while older people are more likely to be 'Subscriber'.
...
# 5. **Trip Duration by User Type**: Customers (casual riders) trips take longer durations than members.
# 6. **Trip Duration Distribution**: Customers take longer duration trips than Subscribers as most trips that are longer than 1500 seconds are taken by customers. On the other hand, most subscribers trips are less than 1000 seconds.
...
# 8. **Peak Trip Times by User Type**: Subscribers are most likely to take trips at the times of going to and leaving work (nine and five). Customers trips are usually between 10 am and 7 pm. They peak at around 3 pm.
# 9. **Trips Weekly Distribution**: The number of trips taken by subscribers is almost stable throught week days (Monday to Friday) and then decreases in weekends (Saturday and Sunday), which would suggest commuting. On the other hand, customers trips were at their highest on weekends and less so on weekdays. 
# 10. **Top Stations**: For subscribers, the top stations (start and end) are: 'Clinton St & Washington Blvd', 'Clinton St & Madison St' and 'Canal St & Adams St'. While for customers, the top stations (start and end) are: 'Streeter Dr & Grand Ave', 'Lake Shore Dr & Monroe St', 'Millennium Park' & 'Shedd Aquarium'. These stations should be targeted with ads for customers to be converted to subscribers. 
# 12. **Missing Birthyear Data**: The 'Customer' data is missing a lot (about three quarters) of 'birthyear' data. The company should try to gather more birthyear data from customers.
# 13. **Age Distribution**:Younger people are more likely to be 'Customer' while older people are more likely to be 'Subsciber'. Most people below thirty years are Customers. Most people older than sixty years are Subscibers. 
